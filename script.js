        ...
        function renderCheckoutItems() {
            ...
            list.innerHTML = checkoutItems.map(item => {
                ...
                return '<div class="checkout-item-card">' +
                    '<img class="checkout-item-img" src="' + item.gambar + '">' +
                    '<div class="checkout-item-info">' +
                        '<div class="checkout-item-name">' + item.nama + '</div>' +
                        '<div class="checkout-item-qty">' + item.qty + ' x Rp' + item.harga.toLocaleString('id-ID') + '</div>' +
                    '</div>' +
                    '<div class="checkout-item-price">Rp' + (item.harga * item.qty).toLocaleString('id-ID') + '</div>' +
                '</div>';
            }).join('');
...
async function submitCheckout() {
      ...
      function showSuccessAnimation(title, desc) {
            ...
            document.getElementById('successLabel').textContent = 'BERHASIL';
            document.getElementById('successTitleMain').textContent = title;
            document.getElementById('successPanelTitle').textContent = 'Terima Kasih!';
            document.getElementById('successPanelText').textContent = desc;

            const actionBtn = document.getElementById('successActionBtn');
            actionBtn.textContent = 'Oke, Mengerti';
            actionBtn.onclick = hideScheduleOverlay;

            document.getElementById('successCloseBtn').style.visibility = 'visible';
        ...
        function showScheduleLoading() {
            ...
            document.getElementById('successLabel').textContent = 'MENJADWALKAN PENJEMPUTAN';
            document.getElementById('successTitleMain').textContent = 'Mengirim Jadwal Kamu...';
            document.getElementById('successPanelTitle').textContent = 'Mohon Tunggu Sebentar';
            document.getElementById('successPanelText').textContent = 'Kami sedang menyimpan jadwal penjemputanmu, jangan tutup halaman ini ya.';

            const actionBtn = document.getElementById('successActionBtn');
            actionBtn.textContent = 'Tunggu Sebentar';
            actionBtn.onclick = null;

            document.getElementById('successCloseBtn').style.visibility = 'hidden';
        ...
        function showScheduleSuccess(title, desc) {
            ...
            badge.classList.remove('loading');
            check.classList.add('show');
            document.getElementById('successLabel').textContent = 'BERHASIL DIJADWALKAN';
            document.getElementById('successTitleMain').textContent = title;
            document.getElementById('successPanelTitle').textContent = 'Terima Kasih!';
            document.getElementById('successPanelText').textContent = desc;

            const actionBtn = document.getElementById('successActionBtn');
            actionBtn.textContent = 'Lihat Status';
            actionBtn.onclick = hideScheduleOverlay;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Sebelumnya, fetch data ini baru dipanggil lewat initCustomer(), yang
    // hanya jalan SETELAH login sukses (baik login manual maupun auto-login
    // dari sesi tersimpan). Saat customer refresh (F5) dan sudah punya sesi
    // tersimpan, menu Toko/Berita terasa "instan" karena localStorage sudah
    // berisi cache dari sesi sebelumnya (fetch sebelumnya sempat selesai).
    // Tapi pada login PERTAMA KALI di perangkat (localStorage masih kosong
    // sama sekali), begitu customer langsung klik menu Toko/Berita setelah
    // login, fetch pertama itu belum sempat selesai — makanya sempat
    // muncul "Memuat...".
    // Dengan memulai fetch ini sejak halaman dibuka (saat customer masih
    // mengisi form Masuk / belum sempat klik apa-apa), data biasanya SUDAH
...
// localStorage sudah terisi sebelum customer sempat membuka menu tsb, baik
// itu lewat login baru maupun lewat sesi yang sudah tersimpan (refresh).
// Aman dipanggil berkali-kali karena masing-masing fungsi sudah punya
// pengecekan cache sendiri (tidak akan fetch ganda yang saling tabrakan).
...
// 🔑 FIX: variabel ini dipakai di loadBannersToSimart() tapi sebelumnya TIDAK
// PERNAH dideklarasikan di manapun (beda dengan simartProductCache/adminNewsCache
// yang sudah benar dideklarasikan). Akibatnya, `if (!simartBannerCache)` di baris
// pertama loadBannersToSimart() melempar ReferenceError setiap kali fungsi itu
// dipanggil pertama kali pada sebuah page-load baru (baik itu login baru maupun
// refresh) — jadi promise-nya reject secara diam-diam dan banner GAGAL dirender.
var simartBannerCache = null;
function saveSimartBannerCache(banners) {
    try { localStorage.setItem('sisaSimartBannerCache', JSON.stringify(banners)); } catch(e) {}
}
function getSimartBannerCache() {
    try { return JSON.parse(localStorage.getItem('sisaSimartBannerCache') || 'null'); } catch(e) { return null; }
}
...
async function loadBannersToSimart() {
    ...
    // 1. Tampilkan cache dulu (kalau ada) — instan, tanpa skeleton
    if (!simartBannerCache) simartBannerCache = getSimartBannerCache();
    if (simartBannerCache) renderSimartBanners(simartBannerCache, track, dotsWrap);

    // 2. Tetap ambil data terbaru dari server di belakang layar
    try {
        var resp = await fetch(APPS_SCRIPT_URL + '?type=list_banners&t=' + Date.now());
        var json = await resp.json();
        var banners = (json.banners || []).slice(0, 4);
        simartBannerCache = banners;
        saveSimartBannerCache(banners);
        renderSimartBanners(banners, track, dotsWrap);
    } catch (err) {
...
function renderSimartBanners(banners, track, dotsWrap) {
    if (!banners.length) {
        track.innerHTML = '<div class="simart-banner-card">' +
            '<div class="simart-banner-overlay">' +
                '<button class="simart-banner-btn" onclick="showApp(\'sipick\')">Belanja Sekarang</button>' +
